...
import { Menu, X } from 'lucide-react'; // keep Lucide just for menu controls
import { FaFacebookF, FaInstagram, FaLinkedinIn, FaTwitter, FaTiktok } from 'react-icons/fa';
...
const Header: React.FC<HeaderProps> = ({ onBookNowClick }) => {
  ...
  const navLinkClasses =
    "relative text-gray-800 hover:text-gray-900 transition-colors duration-300 py-2 group text-base font-medium drop-shadow-sm";
  const activeNavLinkClasses = "text-primary !font-semibold";
  const headerClasses =
    "fixed top-0 w-full z-50 transition-all duration-300 bg-white/20 backdrop-blur-md shadow-sm";
            ...
            <div className="pt-4 border-t border-gray-200">
              ...
              <div className="flex justify-center space-x-6">
                ...
                <a
                  ...
                  className="p-2 rounded-full bg-primary/10 hover:bg-primary/20 transition-colors duration-300"
                >
                  <FaTiktok size={20} color="rgb(22 163 74)" />
                </a>
              </div>
            </div>
          </div>
